    ...
    public class GameAppService : ProfileBaseAppService, IGameAppService
    {
        ...
        public async Task<OperationResultVo<GameViewModel>> GetById(Guid currentUserId, Guid id, bool forEdit)
        {
            try
            {
                ...
                vm.ThumbnailUrl = SetFeaturedImage(vm.UserId, vm.ThumbnailUrl, ImageRenderType.Full);
        ...
        public async Task<OperationResultVo> Comment(Guid currentUserId, CommentViewModel vm)
        {
            try
            {
                ...
                AddCommentGameCommand command = new AddCommentGameCommand(vm.UserId, vm.UserContentId, vm.ParentCommentId, vm.Text , vm.StarRating);
