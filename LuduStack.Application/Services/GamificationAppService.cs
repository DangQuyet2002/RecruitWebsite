...
namespace LuduStack.Application.Services
{
	public class GamificationAppService : IGamificationAppService
	{
			...
			try
			{
				...
				foreach (RankingVo item in allModels)
				{
					RankingViewModel vm = new RankingViewModel
					{
						...
						XpCurrentLevelMax = item.Level != null ? item.Level.XpToAchieve : 1,
						CurrentLevelName = item.Level != null ? item.Level.Name : ""
					};
        ...
        public async Task<OperationResultVo<int>> Count(Guid currentUserId)
        {
            try
            {
                int count = await mediator.Query<CountRankQuery, int>(new CountRankQuery());
