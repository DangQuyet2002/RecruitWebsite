...
using MetaG.Infra.CrossCutting.IoC;
...
MetaGConfigInitializerExtension.UseMetaGConfiguration(builder.Configuration);
...
builder.Services.AddMetaGAutoMapperSetup();
...
builder.Services.AddMediatR(x => x.RegisterServicesFromAssemblies(
	...
	typeof(SendNotificationRequestHandler).Assembly,
	typeof(DeleteCourseCommandHandler).Assembly,
	typeof(GetListGameCategoryQueryHandler).Assembly,
    typeof(GetPaidGamesQueryHandler).Assembly
    ));
...
static void RegisterServices(IServiceCollection services)
{
	...
	MetaGInjectorBootStrapper.RegisterServices(services);
}
