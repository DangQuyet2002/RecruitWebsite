...
namespace LuduStack.Web.Controllers
{
	...
	public class GameController : SecureBaseController
	{
        ...
        [AllowAnonymous]
        public async Task<IActionResult> UpdateNumberPlays(Guid id)
		{
            OperationResultVo<GameViewModel> serviceResult = await gameAppService.GetById(CurrentUserId, id);

			CurrentUserId = serviceResult.Value.UserId;

			serviceResult.Value.NumberPlays += 1;

            await gameAppService.Save(CurrentUserId, serviceResult.Value);

            return RedirectToAction("Index", "Home");
        }

        [AllowAnonymous]
		[Route("categories/{take:int}/{categoryId:guid}")]
		public async Task<IActionResult> ListAllCategories(int take, Guid? categoryId)
		{
			var categories = await metaGameAppService.GetGameCategory(categoryId);

			foreach (var category in categories)
			{
				category.LatestGames = await metaGameAppService.GetGameOfCategory(category.Genre, take);
			}
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("game/GameOutstanding")]
        public async Task<IActionResult> GameOutstanding()
        {
			IEnumerable<Game> latest = await metaGameAppService.GetListGameOutstanding();
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("game/GameMostplayers")]
        public async Task<IActionResult> GameMostplayers()
        {
            var latest = await metaGameAppService.GetPaidGames();
            
            var gameIdsList = latest.Select(game => game.GameId).ToList();

            var RelatedGames = await metaGameAppService.GetListGameMostplayers(gameIdsList);

            return View(RelatedGames);
        ...
        [HttpGet]
        [AllowAnonymous]
        [Route("game/GetPaidGames")]
		public async Task<IActionResult> GetPaidGames()
		{
			var latest = await metaGameAppService.GetPaidGames();

			foreach (var game in latest)
			{
				game.RelatedGames = await metaGameAppService.GetGameOfGameFee(game.GameId);
			}
		...
		public async Task<IActionResult> Details(Guid id, Guid notificationclicked, bool refreshImages)
		{
            ...
            var RelatedGames = await metaGameAppService.GetGameOfCategory(vm.Genre, 0);
		...
		public async Task<IActionResult> Edit(Guid id)
		{
			...
			viewModel.NumberPlays = viewModel.NumberPlays;
        ...
        [HttpPost("game/comment")]
        public async Task<IActionResult> Comment(CommentViewModel vm)
        {
            OperationResultVo response;

            response = await gameAppService.Comment(CurrentUserId, vm);
