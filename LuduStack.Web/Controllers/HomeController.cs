...
namespace LuduStack.Web.Controllers
{
    public class HomeController : SecureBaseController
    {
        ...
        public HomeController(IFeaturedContentAppService featuredContentAppService,
            IGamificationAppService gamificationAppService, IGameAppService gameAppService,
            IMetaGameAppService metagameAppService,
            IBannerAppService bannerAppService, IProfileAppService profileAppService)
        {
        ...
        public async Task<IActionResult> Index(GameGenre genre)
        {
            ...
            var viewModel = new DataHomeViewModel();
            ...
            viewModel.MostPlayers = await metaGameAppService.GetListGameMostplayers(gameIdsList);

            //Danh Sách Game có Phí
            var paidGames = await metaGameAppService.GetPaidGames();
            ...
            viewModel.GetPaidGames = paidGames;

            //Game Nhiều Người chơi Nhất
            viewModel.GameOutstanding = await metaGameAppService.GetListGameOutstanding();

            // Danh Sách Danh Mục
            viewModel.ListCategory = await metaGameAppService.GetListCategory(CurrentUserId, 200, Guid.Empty, 0);

            // Danh sách Banner
            OperationResultListVo<BannerViewModel> banner = await bannerAppService.GetAll();
            viewModel.ListBanner = (List<BannerViewModel>)banner.Value;

            // Số lượt User , Rank , Game
            ...
            OperationResultVo<int> rank = await gamificationAppService.Count(CurrentUserId);

            HttpContext.Session.SetInt32("GamesCount", gamesCount.Value);
            HttpContext.Session.SetInt32("UsersCount", usersCount.Value);
            HttpContext.Session.SetInt32("Rank", rank.Value);
